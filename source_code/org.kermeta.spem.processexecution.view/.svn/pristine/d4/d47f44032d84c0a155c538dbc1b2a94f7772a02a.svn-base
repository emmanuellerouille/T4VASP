...
public class DoAction extends ProcessAction{
	...
	public void run() {
		...
		//Set the name of the work unit under execution, if it has not already been done
		if(!ModelUtils.isInitializationAutomated(taskUseName) && !ModelUtils.isInitializationManual(taskUseName)){
		...
		if(ModelUtils.isExecutionAutomated(taskUseName)){
			if(!PlatformUI.getWorkbench().getPreferenceStore().getBoolean("before restart")){
				Utils.computeTime(beginning.getTime(), contextModelPath, taskUseName, "on do");
			}
