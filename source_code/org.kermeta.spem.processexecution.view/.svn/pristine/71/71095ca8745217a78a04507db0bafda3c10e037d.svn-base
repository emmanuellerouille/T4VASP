...
public class Utils {
				...
				for( int i = 0; i < taskFigure.getStdActionsPlate().getChildren().size(); i++){
					...
					if(label instanceof HyperlinkFigure){
						...
						if(hyperlinkFigure.getText() == "on start"){
							if(ModelUtils.isInitializationAutomated(workUnit.getName()) || ModelUtils.isInitializationManual(workUnit.getName())){
	...
	public static void setStatusFollowingWorkUnitsEndNode(ProcessPart view, ExecutionContext executionContext, Activity containingActivity){
		if(!(containingActivity instanceof DeliveryProcess)){//If the containing activity is not an instance of DeliveryProcess, then return to the execution of the containing activity
		...
		}else{//If the containing activity is an instance of DeliveryProcess, then finish the process execution
	...
	public static void initProcessView(ExecutionContext executionContext, ArrayList<String> taskFigureNames, Container container, Activity containingActivity){
		try {
			...
			for(int i = 0; i <view.getMediator().getView().getTaskFigures().size(); i++){
				...
				addAction(taskFigure, ModelUtils.getContextModelPath(), false, false, false, false, false, false, false, containingActivity);
			}
	...
	public static void setActionsAvailability(TaskFigure taskFigure, State state, Boolean optional, Activity containingActivity, String contextModelPath){
		if(state.getValue() == State.NOT_STARTED_VALUE || state.equals(State.FINISHED)){
			addAction(taskFigure, contextModelPath, false, false, false, false, false, false, optional, containingActivity);				
		}else if(state.getValue() == State.ON_START_VALUE){
			addAction(taskFigure, contextModelPath, true, false, false, false, false, false, optional, containingActivity);			
		}else if(state.getValue() == State.END_INITIALIZATION_VALUE){
			addAction(taskFigure, contextModelPath, false, true, false, false, false, false, optional, containingActivity);			
		}else if(state.getValue() == State.ON_DO_VALUE){
			addAction(taskFigure, contextModelPath, false, false, true, false, false, false, optional, containingActivity);
		}else if(state.getValue() == State.DONE_VALUE){
			addAction(taskFigure, contextModelPath, false, false, false, true, false, false, optional, containingActivity);
		}else if(state.getValue() == State.ON_END_VALUE){
			addAction(taskFigure, contextModelPath, false, false, false, false, true, false, optional, containingActivity);
		}else if(state.getValue() == State.END_FINALIZATION_VALUE){
			addAction(taskFigure, contextModelPath, false, false, false, false, false, true, optional, containingActivity);
		}
	}

	public static void addAction(TaskFigure taskFigure, String contextModelPath, Boolean onStart, Boolean endInitialization, Boolean onDo, Boolean done, Boolean onEnd, Boolean endFinalization, Boolean skip, Activity containingActivity){
		...
		taskFigure.addAction(new HyperlinkFigure("end initialization", new EndInitialization(endInitialization, taskFigure.getDescription(), contextModelPath, containingActivity)));
		...
		taskFigure.addAction(new HyperlinkFigure("end finalization", new EndFinalization(endFinalization, taskFigure.getDescription(), contextModelPath, containingActivity)));
