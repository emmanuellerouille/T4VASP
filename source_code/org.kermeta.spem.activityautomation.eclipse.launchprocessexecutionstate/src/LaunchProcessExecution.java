...
public class LaunchProcessExecution implements IStartup {
	...
	public void earlyStartup() {
		...
		if(PlatformUI.getWorkbench().getPreferenceStore().getBoolean("process execution")){
			...
			PlatformUI.getWorkbench().getPreferenceStore().setValue("before restart", false);
			...
			UIJob uiJob = new UIJob("open process view and initialize its state"){
				...
				public IStatus runInUIThread(IProgressMonitor monitor) {
					...
					//If the execution is automated, launch the execution of the ACs that have not been executed because of the restart
					...
					}else if(ModelUtils.isExecutionAutomated(workUnitName)){ 
						if(onWhat.getValue() == OnWhat.ON_DO_VALUE){
					...
					}else if(ModelUtils.isFinalizationAutomated(workUnitName)){
						if(onWhat.getValue() == OnWhat.ON_END_VALUE){
